...
export class ArticleService {
  ...
  async getAllArticles(query: GetArticlesQueryDto) {
    console.log(query);
    const { author, favorited, tag, limit, offset } = query;
    ...
    if (favorited) {
      const favoritedUser = await this.em.findOne(User, {
        username: favorited,
      });
      const articlesFavoritedByUser = await this.em.findOne(Favorite, {
        user: favoritedUser?.id,
      });
      const favoriteArticleIds = articlesFavoritedByUser?.favoriteArticles;

      articles = articles.filter((a) => favoriteArticleIds?.includes(a.id));
    }
    const response = {
  ...
  async getFeedArticles(limit: number, offset: number) {
    ...
    const followedAuthors = await this.em.find(Follows, {
    ...
    const feeds = await this.em.find(
      Article,
      {
        author: { id: { $in: followedAuthorsIds.map((id) => id.toString()) } },
      },
  ...
  async updateArticle(slug: string, updateArticleInput: UpdateArticleInput) {
    ...
    const article = await this.em.findOne(Article, {
  ...
  async unFavoriteArticle(slug: string) {
    ...
    const unfavoriteArticle = await this.em.transactional(async (em) => {
      ...
      if (favorite && favorite.favoriteArticles?.includes(article.id)) {
        favorite.favoriteArticles = favorite.favoriteArticles.filter(
          (id) => id !== article.id,
        );
        article.favoritesCount--;
        await this.em.flush();
