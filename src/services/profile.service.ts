...
export class ProfileService {
  ...
  async fetchProfile(userName: string) {
    ...
    const userInfo = await this.em.findOne(
      User,
      {
        username: userName,
      },
    ...
    const isFollowing = await this.em.findOne(Follows, {
      followedId: userInfo.id,
  ...
  async followUser(userName: string) {
    ...
    const checkIsFollowing = await this.em.findOne(Follows, {
      followedId: userInfo.id,
      followerId: currentUser.id,
    });
    if (checkIsFollowing) {
    ...
    const follow = this.em.create(Follows, {
      followedId: userInfo.id,
      followerId: currentUser.id,
    });
    let following: boolean;
