...
export class UserService {
    ...
    async registerUser(registerDto: RegisterUserInput) {
        try {
            ...
            const userExists = await this.em.findOne(User, { $or: [{ email: userInput.email }, { username: userInput.username }] })
    ...
    async loginUser(input: LoginUserInput) {
        ...
        const user = await this.userRepository.verifyUser(loginDto.email, loginDto.password)
    ...
    async updateUser(input: UpdateUserInput) {
        try {
            ...
            this.logger.log("Logging current userinput ", userDto)

            const currentUser = HttpContext.get().req.user;
            this.logger.log("Logging current user in updateUser", currentUser.id)

            const user = await this.em.findOneOrFail(User, { id: currentUser.id }, { failHandler: () => new BadRequestException() })
