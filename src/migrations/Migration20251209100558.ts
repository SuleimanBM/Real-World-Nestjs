...
export class Migration20251209100558 extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`create table "favorite" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz null, "user" varchar(255) not null, "favorite_articles" text[] null, constraint "favorite_pkey" primary key ("id"));`);
    this.addSql(`alter table "favorite" add constraint "favorite_user_unique" unique ("user");`);

    this.addSql(`create table "follows" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz null, "follower_id" varchar(255) not null, "followed_id" varchar(255) not null, constraint "follows_pkey" primary key ("id"));`);
    this.addSql(`alter table "follows" add constraint "follows_follower_id_followed_id_unique" unique ("follower_id", "followed_id");`);
