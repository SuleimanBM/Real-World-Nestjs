...
describe("Article test suits", () => {
    ...
    it("Create article", async () => {
        ...
        const article = await TestContext.em.findOne(Article, { slug: input.article.slug })
    ...
    it("should get all articles", async () => {
        ...
        const createArticles = await TestContext.getFactory(ArticleFactory).create(10)
    ...
    it("Should update an article", async () => {
        ...
        const createArticle = await TestContext.getFactory(ArticleFactory).createOne({author: createUser.id})

        const updateArticleData = { article: { title: "Testing with Nestjs", } }

        const updatedArticle = await articleService.updateArticle(createArticle.slug!, updateArticleData)
    ...
    describe("Delete article", () => {
        ...
        it("Delete article by author", async () => {
            ...
            const createArticle1 = await TestContext.getFactory(ArticleFactory).createOne({ author: createUser1.id })

            await articleService.deleteArticle(createArticle1.slug!)

            const found = await TestContext.em.findOne(Article, { slug: createArticle1.slug })
        ...
        it("Should throw an error if user is not author of article", async () => {
            ...
            const createArticle = await TestContext.getFactory(ArticleFactory).createOne({ author: createUser2.id })

            expect(articleService.deleteArticle(createArticle.slug!)).rejects.toThrow("Article does not exist")
    ...
    it("Favorite an article", async () => {
        ...
        const favoritedArticle = await articleService.favoriteArticle(article.slug!)
    ...
    it("Unfavorite an article", async () => {
        ...
        const unfavoritedArticle = await articleService.unFavoriteArticle(article.slug!)
    ...
    it.skip('calls the callback', () => {
        ...
        const mockCallback = jest.fn((a, b) => {return a * b});

       // greetUser(mockCallback);

        mockCallback(2, 4)
        expect(mockCallback).toHaveBeenCalled();
        expect(mockCallback).toHaveBeenCalledWith(2, 4); // ✅ true
    });
    test.skip('the house has my desired features', () => {
        const houseForSale = {
            bath: true,
            bedrooms: 4,
            kitchen: {
